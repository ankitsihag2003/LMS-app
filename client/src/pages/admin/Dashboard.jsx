...
const Dashboard = () => {
  ...
  const {data,isLoading,isSuccess,isError} = useGetAllPurchasedCoursesQuery();

  if(isLoading) return <p>Loading...</p>
  if(isError) return <p className='text-red-500'>Failed to load data.</p>

  const {purchasedCourses} = data;
  const courseData = purchasedCourses.map(course => ({name: course.courseId.courseTitle, price: course.courseId.coursePrice}));
